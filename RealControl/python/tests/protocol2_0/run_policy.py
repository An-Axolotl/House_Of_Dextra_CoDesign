...
try:
    while True:
        ...
        if ch in ('\x1b','q'): break

        # --- build goals for this tick ---
        for motor_id in DXL_ID_MOVE:
            goal_pos = motor_limits[motor_id]['mid'] if index == 0 else motor_limits[motor_id]['tight_lo']
            param_goal_position = [
                DXL_LOBYTE(DXL_LOWORD(goal_pos)), DXL_HIBYTE(DXL_LOWORD(goal_pos)),
            ...
            last_goals[motor_id] = goal_pos # remember last goal targets
        ...
        t_after_write = now  # 'now' was captured just before txPacket()
        ...
        # bus round-trip (write -> first read)
        ...
        if cmd_periods and (time.perf_counter() - last_print) >= 1.0:
            ...
            # MOVING motors (read via GroupSyncRead cache)
            for motor_id in sorted(DXL_ID_MOVE):
                goal_t  = last_goals.get(motor_id, 0)
                pres_t  = last_positions.get(motor_id, 0)

                goal_r  = ticks_to_rad(goal_t)
                pres_r  = ticks_to_rad(pres_t)
                err_r   = error_rad(goal_t, pres_t)  # signed, wrapped [-pi, pi)

                print(
                    f"[ID:{motor_id:02d}] MOVING | "
                    f"Goal:{goal_t:4d} ({goal_r:+.4f} rad)  "
                    f"Present:{pres_t:4d} ({pres_r:+.4f} rad)  "
                    f"Err:{err_r:+.4f} rad"
                )

            # MIDDLE motors (single read each tick)
            middle_positions = {}
            for motor_id in DXL_ID_MIDDLE:
            ...
            for motor_id in sorted(DXL_ID_MIDDLE):
                ...
                goal_r  = ticks_to_rad(goal_t)
                pres_r  = ticks_to_rad(pres_t)
                err_r   = error_rad(goal_t, pres_t)

                print(
                    f"[ID:{motor_id:02d}] MIDDLE | "
                    f"Goal:{goal_t:4d} ({goal_r:+.4f} rad)  "
                    f"Present:{pres_t:4d} ({pres_r:+.4f} rad)  "
                    f"Err:{err_r:+.4f} rad"
                )
        ...
        # toggle just for demo; your policy would update goals here
        index ^= 1
